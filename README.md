...
**优点：**
* 其他客户端或者服务器也可以 **主动** 向 DDTV 推送信息
* 实时的信息，频率可以很快 （比如上传进度，等等）
* 无需公网ip 即可实现互联
* 将开发服务端的任务交给用户 

**缺点：**
* 普通的http请求也可以做到 比如做一个轮询
* 很多用户不会去用
* webhook 也可以做到
* ws会因为网络等因素断线
...
```sequence
DDTV(ws客户端)->用户自己实现的ws服务端: 向服务器主动发送数据
用户自己实现的ws服务端 -> DDTV(ws客户端): 服务器 或者 连接在这个服务器上其他客户端 主动推送信息
